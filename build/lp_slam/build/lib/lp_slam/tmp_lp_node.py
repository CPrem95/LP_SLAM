...
def main():
    ...
    est_ln_lms = []
    for i in range(20):
        tmp_plt_ln_all, = ax.plot([], [], c='red', linewidth = 2, linestyle= '-', label='Est Ln LMs')
    ...
    for i in range(0, stepend, stepsize):
        ...
        ## EKF Update
        ############################################################################################################
        mu, sig, N_pts, N_line, hist_i, obs_count, visLine_x, visLine_y = ekf.EKF_unknown_correction_LP(mu_bar, sig_bar, obs_Pt, obs_Lin, Q_pts, Q_lines, i, hist_i, obs_count, exp_pt_landm, exp_line_landm, N_pts, N_line, visLine_x, visLine_y, 1, 5)
        ...
        if slam_plot:
            ...
            # Adjust the axis limits
            set_limits(ax2, np.concatenate((obs_lhs_R12, obs_rhs_R12), axis=0), odom[i:i+stepsize, :])
            fig2.canvas.draw()
            fig2.canvas.flush_events()
