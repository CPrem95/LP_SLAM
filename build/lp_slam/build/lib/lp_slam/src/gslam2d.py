import g2o
import numpy as np
from ekf_slam.src.helper_plotting_functions import plot_slam2d
import math
...
class GraphSLAM2D:
    ...
    def add_landmark2(self, x0, y0, R, information, pose_id, landmark_id=None):
        '''
        Add landmark to the graph
        '''

        est_pose = self.vertex_pose(pose_id)
        x_r = est_pose[0]
        y_r = est_pose[1]
        theta_r = est_pose[2]

        beta = math.atan2(y0 - y_r, x0 - x_r) - theta_r # x0, y0 are the coordinates of the initial landmark estimation
        ...
        if self.verbose:
            print("Adding landmark edge between", pose_id, landmark_id)
            print("Landmark measurement: ", measurement)
